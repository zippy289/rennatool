'use client';
// src/components/bookings/BookingWidget.tsx
...
function calcPricing(dailyRate: number, deposit: number, days: number, weeklyDiscount?: number) {
  ...
  const ownerEarns   = discounted - platformFee;
  return {
    subtotal:       Math.round(discounted),
    discountAmount: Math.round(subtotal * discountPct),
    platformFee,
    ownerEarns,
    total:          Math.round(discounted) + deposit,
  };
}
...
export function BookingWidget({ tool, bookedRanges }: BookingWidgetProps) {
  ...
  const leadBlanks = monthStart.getDay(); // 0 = Sun
  ...
  const pricing   = totalDays > 0
    ? calcPricing(tool.dailyRate, tool.depositAmount, totalDays, tool.weeklyDiscount)
    : null;
  ...
  const handleDayClick = (d: Date) => {
    ...
    if (!start || (start && end)) {
      ...
      if (span.some((day) => isBooked(day, bookedRanges))) {
        setStart(d); setEnd(null);
      } else {
        setEnd(d);
      }
    ...
    <div className="bg-white border border-sand rounded-2xl overflow-hidden shadow-card">
      ...
      <div className="px-5 pt-5 pb-4 border-b border-sand">
        <div className="flex items-baseline gap-1">
          <span className="text-[24px] font-bold" style={{ fontFamily:'var(--font-display)' }}>
            {fmt(tool.dailyRate)}
          </span>
          <span className="text-[13px] text-ink-subtle">/day</span>
        </div>
        {tool.weeklyDiscount && (
          <p className="text-[12px] text-green-600 mt-0.5 font-medium">
            {tool.weeklyDiscount}% off 7+ day rentals
          </p>
        )}
      </div>
      ...
      <div className="px-5 py-4">
        <div className="flex items-center justify-between mb-3">
          <button onClick={() => setMonth(subMonths(month, 1))} className="p-1.5 rounded-lg hover:bg-canvas transition-colors">
            <ChevronLeft className="w-4 h-4 text-ink-subtle" />
          </button>
          <span className="text-[13px] font-semibold">{format(month, 'MMMM yyyy')}</span>
          <button onClick={() => setMonth(addMonths(month, 1))} className="p-1.5 rounded-lg hover:bg-canvas transition-colors">
        ...
        <div className="grid grid-cols-7 mb-1">
          {['Su','Mo','Tu','We','Th','Fr','Sa'].map((d) => (
            <div key={d} className="text-center text-[11px] text-ink-subtle py-1">{d}</div>
          ))}
        </div>
          ...
          {days.map((day) => {
            ...
            return (
              <button
                ...
                className={`
                  relative h-8 w-full text-[12px] rounded-lg transition-all
                  ${disabled  ? 'text-ink-subtle/40 cursor-not-allowed line-through' : 'cursor-pointer'}
                  ${booked    ? 'bg-red-50 text-red-300' : ''}
                  ${selected  ? 'bg-brand text-white font-semibold' : ''}
                  ${inSel     ? 'bg-brand/15 text-brand-dark rounded-none' : ''}
                  ${!disabled && !selected && !inSel ? 'hover:bg-canvas' : ''}
                  ${isToday(day) && !selected ? 'font-bold text-brand' : ''}
                `}
              >
                {format(day, 'd')}
              </button>
        ...
        {start && end && (
          <p className="text-[12px] text-center text-ink-subtle mt-2">
            {format(start,'MMM d')} – {format(end,'MMM d')} · {totalDays} day{totalDays!==1?'s':''}
          </p>
        )}
      </div>
      ...
      {pricing && (
        <div className="px-5 py-4 border-t border-sand space-y-2 text-[13px]">
          <div className="flex justify-between text-ink-subtle">
            <span>{fmt(tool.dailyRate)} × {totalDays} days</span>
            <span>{fmt(tool.dailyRate * totalDays)}</span>
          </div>
          ...
          <div className="flex justify-between text-ink-subtle pt-2 border-t border-sand">
            <span className="flex items-center gap-1">
              <Shield className="w-3.5 h-3.5" /> Refundable deposit
            </span>
            <span>{fmt(tool.depositAmount)}</span>
          </div>
          <div className="flex justify-between font-semibold text-[14px] pt-2 border-t border-sand">
            <span>Total due</span>
      ...
      <div className="px-5 pb-5">
        <button
          onClick={handleBook}
          disabled={!tool.available || !start || !end || loading ||
            (totalDays < tool.minRentalDays) || (totalDays > tool.maxRentalDays)}
          className="
        ...
        >
          {loading ? 'Processing…' :
           !tool.available ? 'Currently unavailable' :
           !start || !end ? 'Select dates to continue' :
           totalDays < tool.minRentalDays ? `Minimum ${tool.minRentalDays} days` :
           totalDays > tool.maxRentalDays ? `Maximum ${tool.maxRentalDays} days` :
           'Request to book'}
        </button>
        <p className="text-[11px] text-center text-ink-subtle mt-2">
