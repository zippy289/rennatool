...
import { CheckCircle, Clock, Package, Star, Search } from 'lucide-react';
      ...
      <main className="flex-1">
        <div className="max-w-4xl mx-auto px-6 lg:px-10 py-10">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-[32px] font-bold tracking-tight" style={{ fontFamily: 'var(--font-display)' }}>My rentals</h1>
              <p className="text-[13px] text-ink-subtle mt-1 font-light">Track your active and past tool rentals</p>
            </div>
            <Link href="/search" className="flex items-center gap-2 px-4 py-2.5 border border-sand bg-white hover:bg-canvas text-ink rounded-lg text-[13px] font-medium transition-colors">
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            {[
              ...
              <div key={label} className="bg-white border border-sand rounded-xl p-4 text-center">
                ...
                <p className="text-[11px] text-ink-subtle mt-0.5 uppercase tracking-wide" style={{ fontFamily: 'var(--font-mono)' }}>{label}</p>
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-1 p-1 bg-white border border-sand rounded-xl mb-6 w-fit">
            {TABS.map((t) => (
              <button key={t} onClick={() => setTab(t)} className={`px-4 py-1.5 rounded-lg text-[13px] font-medium capitalize transition-all ${tab === t ? 'bg-ink text-white' : 'text-ink-subtle hover:text-ink'}`}>{t}</button>
            ))}
          </div>
          ...
          {filtered.length > 0 ? (
                ...
                return (
                  <div key={booking.id} className="bg-white border border-sand rounded-2xl p-5 hover:shadow-card transition-shadow">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-xl bg-canvas flex items-center justify-center text-2xl flex-shrink-0">{booking.emoji}</div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[13px] text-ink-subtle font-light">{booking.owner} · {booking.dates}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="font-semibold text-[15px]">{fmt(booking.total)}</p>
                        <p className="text-[11px] text-ink-subtle mt-0.5">+ {fmt(booking.depositAmount)} deposit</p>
                      </div>
                    </div>
                    {(booking.status === 'ACTIVE' || booking.status === 'PAID') && (
                      <div className="flex gap-2 mt-4 pt-4 border-t border-sand">
                        <Link href={`/bookings/${booking.id}`} className="px-4 py-2 bg-ink text-white rounded-lg text-[12px] font-medium hover:bg-ink-2 transition-colors">View details</Link>
                        {booking.status === 'ACTIVE' && <button className="px-4 py-2 border border-sand rounded-lg text-[12px] text-ink-subtle hover:bg-canvas transition-colors">Report return</button>}
                      </div>
                    )}
                    {booking.status === 'COMPLETED' && (
                      <div className="flex items-center gap-2 mt-4 pt-4 border-t border-sand">
                        <button className="flex items-center gap-1.5 px-4 py-2 border border-sand rounded-lg text-[12px] text-ink-subtle hover:bg-canvas transition-colors">
                          <Star className="w-3.5 h-3.5 text-amber-400" /> Leave a review
                        </button>
                        <Link href="/search" className="px-4 py-2 text-[12px] text-brand hover:underline font-medium">Rent again →</Link>
                      </div>
                    )}
                  </div>
                );
            ...
            <div className="text-center py-16 bg-white rounded-2xl border border-sand">
              ...
              <h3 className="text-[16px] font-semibold mb-2" style={{ fontFamily: 'var(--font-display)' }}>No rentals here</h3>
              <p className="text-[13px] text-ink-subtle mb-5 font-light">{tab === 'active' ? 'You have no active rentals right now.' : tab === 'upcoming' ? "You don't have any upcoming bookings." : "You haven't rented any tools yet."}</p>
              <Link href="/search" className="text-[13px] text-brand hover:underline font-medium">Browse tools near you →</Link>
            </div>
          )}
        </div>
      </main>
