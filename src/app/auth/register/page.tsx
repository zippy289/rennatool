    ...
    <div className="min-h-screen bg-canvas flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-[400px]">
        ...
        <div className="bg-white border border-sand rounded-2xl p-8 shadow-card">
          <h1 className="text-[26px] font-bold mb-1 tracking-tight" style={{ fontFamily: 'var(--font-display)' }}>Create your account</h1>
          <p className="text-[13px] text-ink-subtle font-light mb-7">Free to join. Start renting or listing tools today.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input id="reg-email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" autoComplete="email"
                className="w-full px-3.5 py-2.5 border border-sand rounded-lg text-[14px] outline-none focus:border-brand transition-colors placeholder:text-ink-subtle/50" />
            </div>
            <div>
              <label className="block text-[13px] font-medium mb-1.5" htmlFor="reg-password">Password</label>
              <div className="relative">
                <input id="reg-password" type={showPw ? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Create a password" autoComplete="new-password"
                ...
                <div className="mt-2 space-y-1">
                  {REQUIREMENTS.map(({ test, label }) => (
                    <div key={label} className={`flex items-center gap-1.5 text-[11px] ${test(password) ? 'text-green-600' : 'text-ink-subtle'}`}>
                      <Check className={`w-3 h-3 ${test(password) ? 'text-green-500' : 'text-sand'}`} />{label}
                    </div>
                  ))}
                </div>
              )}
            </div>
            <button type="submit" disabled={loading} className="w-full py-3 bg-brand hover:bg-brand-dark text-white rounded-lg text-[14px] font-medium transition-colors disabled:opacity-60 mt-2">
          ...
          <p className="text-center text-[13px] text-ink-subtle mt-4">
            Already have an account? <Link href="/auth/login" className="text-brand hover:underline font-medium">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
