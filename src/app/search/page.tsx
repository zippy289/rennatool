...
const SORT_OPTIONS = [
  { value: 'distance', label: 'Nearest first' }, { value: 'price_asc', label: 'Price: low to high' },
...
export default function SearchPage() {
  ...
  const [filters, setFilters] = useState({ category: searchParams.get('category') ?? '', minPrice: '', maxPrice: '', radius: '25' });
        ...
        <div className="max-w-6xl mx-auto px-6 lg:px-10 py-4">
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search tools…"
              ...
              {query && <button type="button" onClick={() => setQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#6B5E52]"><X className="w-3.5 h-3.5" /></button>}
            </div>
            <div className="relative sm:w-[120px]">
              ...
              <input type="text" value={zip} onChange={(e) => setZip(e.target.value.replace(/\D/g, '').slice(0, 5))} placeholder="ZIP"
            ...
            <button type="submit" className="px-5 py-2.5 bg-brand hover:bg-brand-dark text-white rounded-lg text-sm font-medium transition-colors">Search</button>
          </form>
        </div>
      </div>
      <main className="flex-1">
        <div className="max-w-6xl mx-auto px-6 lg:px-10 py-8">
          <div className="flex gap-8">
            <aside className="hidden lg:block w-60 flex-shrink-0"><SearchSidebar filters={filters} onFiltersChange={setFilters} /></aside>
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-6">
                <p className="text-[15px] font-medium">{results.length} tool{results.length !== 1 ? 's' : ''} found{zip ? ` near ${zip}` : ''}</p>
                <div className="flex items-center gap-2">
                  <button onClick={() => setSidebarOpen(true)} className="lg:hidden flex items-center gap-1.5 px-3 py-2 border border-sand rounded-lg text-sm hover:bg-white transition-colors">
                    <SlidersHorizontal className="w-4 h-4" /> Filters
                  </button>
                  <select value={sort} onChange={(e) => setSort(e.target.value)} className="px-3 py-2 border border-sand rounded-lg text-sm bg-white outline-none focus:border-brand cursor-pointer" style={{ fontFamily: 'var(--font-body)' }}>
              ...
              {results.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                  {results.map((tool, i) => (
                    <div key={tool.id} className="animate-fade-up" style={{ animationDelay: `${i * 50}ms` }}><ToolCard tool={tool} /></div>
                  ))}
                </div>
              ) : (
                <div className="text-center py-20 bg-white rounded-2xl border border-sand">
                  <div className="text-5xl mb-4">🔧</div>
                  <h3 className="text-lg font-semibold mb-2" style={{ fontFamily: 'var(--font-display)' }}>No tools found</h3>
                  <p className="text-sm text-ink-subtle mb-5">Try adjusting your search or expanding the radius.</p>
                  <button onClick={() => { setQuery(''); setFilters({ category: '', minPrice: '', maxPrice: '', radius: '25' }); }} className="text-sm text-brand hover:underline font-medium">Clear all filters</button>
                </div>
              )}
            </div>
          </div>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-80 bg-white shadow-deep overflow-y-auto animate-slide-down">
            ...
            <div className="p-4"><SearchSidebar filters={filters} onFiltersChange={(f) => { setFilters(f); setSidebarOpen(false); }} /></div>
          </div>
        </div>
      )}
      <Footer />
